...
- **Loop and binding variables get full names too — "small scope" is no excuse.** Name the iteration variable for the singular of what it holds: `for order in orders`, `for event in events`, `for record in records` — never `o`, `ev`, `rec`, or a bare `x`. The same applies to a value pulled from a call (`order = build(...)`, not `o`) and to comprehension variables (`[result for result in fetch()]`, not `[r for r in fetch()]`). A short scope is not a license to abbreviate — the reader still has to know what `o` is. The only exceptions: a deliberately-unused binding (`_`), and conventional `i`/`j` for a pure numeric `range()` with no domain meaning.
