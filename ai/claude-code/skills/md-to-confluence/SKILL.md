...
- **Mermaid `<br/>` only lives inside `Note` blocks.** A `<br/>` on a message-arrow line (`A-->>B: text<br/>more`) is a parse error and the whole diagram fails to render. If the render errors on a `<br/>`, move that text into a `Note over ...:` line or drop the break. This is the single most common failure.
