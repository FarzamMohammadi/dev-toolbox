...
**Steps:**
1. Read the content they share
2. Reflect the intent back in your own words: "Here's what I'm hearing you want to convey..."
3. Get confirmation before proceeding
4. If the user gives context about their work or career (even rambling voice-to-text), absorb every detail. This is gold. It tells you what matters to them, what they're proud of, and what they want a reader to feel.
...
**Steps:**
1. Propose 3-4 completely different structural approaches
2. Each must be a genuinely distinct framing, not minor rewording of the same idea
3. Present them as full drafts so the user can feel the difference in rhythm and emphasis
4. User picks a direction
5. Once picked, the structure is LOCKED. All further refinement happens within it.
...
**What to do when the user picks a direction but isn't fully satisfied:**
They've locked the framing (the "way we say it"), not the content (the "what we say"). Move to Phase 3 where they get granular control over each piece.
...
**For bullet points, first identify what "jobs" the bullet is doing:**
Read the bullet and list every distinct thing it's trying to communicate. A common finding: one bullet is carrying 5-7 jobs. That's why it's 80 words. Example:
...
1. Assemble the pieces into the full text
2. Show the assembled version
3. Read it aloud (mentally). Does it flow? Are there jarring transitions between pieces?
4. If a combination doesn't work in assembly, flag it and offer alternatives for the problematic seam

**Going deeper on individual words within a piece:**
If the user likes a piece's structure but not a specific word, break it down further. Isolate the word and offer replacements:
...
**Self-containment test (bullets only):**
After assembly, ask: if you deleted every other bullet in this section, would this one still make complete sense? If it references work described in another bullet, or uses "this" to refer to something introduced elsewhere, it fails the test. Fix it before moving on.
...
These words appear on so many resumes that they carry zero signal. Challenge them directly:
...
If the summary or section has room for a few more words, propose surgical additions that add real signal, not padding. Each addition must pass the test: "does this word/phrase add information a reader couldn't infer?"

Good additions: restoring a compressed list that shows depth, adding an architecture descriptor ("plugin-driven"), specifying a scope qualifier.
Bad additions: filler adjectives, redundant clarifications, hedging language.
